...
class Bill_App:
    def __init__(self, root):
        ...
        lbl_title=Label(self.root,text="CROP MANAGEMENT SYSTEM",font=("times new roman",35,"bold"),bg="white",fg="blue")
        ...
        def my_time(self):
                ...
                lbd.config(text=date_string)
                self.dt=date_string
                print(self.dt)


        lbd=Label(lbl_title,font=("times new roman",16,"bold"),background='white',foreground='blue')
        lbd.place(x=135,y=(-15),width=120,height=50) 
        ...
        self.trv = ttk.Treeview(MiddleFrame, columns=(1,2,3,4), height=14, show="headings")
        self.trv.column(1, anchor=CENTER, stretch=NO, width=230)
        self.trv.column(2, anchor=CENTER, stretch=NO, width=230)
        self.trv.column(3, anchor=CENTER, stretch=NO, width=230)
        self.trv.column(4, anchor=CENTER, stretch=NO, width=230)

        self.trv.heading(1, text="Phone No")
        self.trv.heading(2, text="Name")
        self.trv.heading(3, text="Email")
        self.trv.heading(4, text="Bill no")

        self.conn = mysql.connector.connect(host='localhost', port='3307', username='root', password='0000', database='reg')
...
###-------------------------------
        ...
        self.search_bill=ttk.Entry(Serach_Frame,textvariable=self.find_bill,font=("arial",10,"bold"),width=24)
        self.search_bill.grid(row=0,column=1,stick=W,padx=2)
        ...
        RightLabelFrame.place(x=1020,y=45,width=460,height=440)
        ...
        self.BtnAddToCart=Button(Btn_Frame,command=self.AddItem,height=2,text="Add to cart",font=('arial',15,'bold'),bg="blue",fg="white",width=15,cursor="hand2")
        self.BtnAddToCart.grid(row=0,column=0)

        self.Btngenerate=Button(Btn_Frame,command=self.gen_bill,height=2,text="Generate Bill",font=('arial',15,'bold'),bg="blue",fg="white",width=15,cursor="hand2")
        self.Btngenerate.grid(row=0,column=1)

        self.BtnSave=Button(Btn_Frame,height=2,command=self.save_bill,text="Save Bill",font=('arial',15,'bold'),bg="blue",fg="white",width=15,cursor="hand2")
        self.BtnSave.grid(row=0,column=2)

        self.BtnPrint=Button(Btn_Frame,height=2,command=self.iprint,text="Main Menu",font=('arial',15,'bold'),bg="blue",fg="white",width=15,cursor="hand2")
        self.BtnPrint.grid(row=0,column=3)

        self.BtnClear=Button(Btn_Frame,height=2,command=self.clear,text="Clear",font=('arial',15,'bold'),bg="blue",fg="white",width=15,cursor="hand2")
        self.BtnClear.grid(row=0,column=4)

        self.BtnExit=Button(Btn_Frame,height=2,command=self.root.destroy,text=" Exit",font=('arial',15,'bold'),bg="blue",fg="white",width=15,cursor="hand2")
    ...
    def display_users(self):
        ...
        self.my_cursor.execute("SELECT * FROM `nbill`")
        users = self.my_cursor.fetchall()
    ...
    def AddItem(self):
            ...
            self.tc = self.qty.get()
            self.c.append(self.tc)
    
            crop_name = self.product.get()
            quantity = self.qty.get()
    
            if self.product.get() == "":
             messagebox.showerror("ERROR", "Select product name")
            else:
                command = "SELECT quantity FROM crops WHERE cropname = %s"
                my_cursor.execute(command, (crop_name,))

                rows = my_cursor.fetchall()

                if rows:
                    for row in rows:
                        fetched_quantity = row[0]
                        print("qty:",fetched_quantity)
                
                if(fetched_quantity>0):
                                    self.gen()
                            ...
                            messagebox.showinfo("Stock:", "Quantity not available...\nAvailable Quatity :"+str(fetched_quantity))
                            print(type(fetched_quantity))
    ...
    def gen(self):
        try:
            ...
            if rows:
                for row in rows:
                    fetched_quantity = row[0]
                    print(fetched_quantity)
                    fetched_quantity=fetched_quantity-quantity
                    print(fetched_quantity)

                    if(fetched_quantity>=0):

                        command = "UPDATE crops SET quantity = %s WHERE cropname = %s"
                        my_cursor.execute(command, (fetched_quantity, crop_name))
                        conn.commit()
            ...
            print("Error:", e)
            messagebox.showerror("Inventory", "An error occurred. Please try again.")
    ...
    def add_item(self):
        ...
        try:
            ...
            command = "INSERT INTO nbill(phno,cname, email,billno) VALUES (%s,%s,%s,%s)"
            my_cursor.execute(command, (ph,cn,mail ,bill))
            conn.commit()
